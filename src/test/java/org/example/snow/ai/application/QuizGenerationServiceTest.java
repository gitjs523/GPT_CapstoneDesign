...
class QuizGenerationServiceTest {
    ...
    @Test
    void savesContextsAndQuizzesAndCompletesJob() {
        Notebook notebook = createNotebook(1L, 10L);
        Section section = createSection(notebook, 100L);
        GenerationJob job = createJob(notebook, 55L, "RAG 단원", 2);
        ...
        verify(ollamaService, times(2)).generateQuiz(promptCaptor.capture());
    ...
    private GenerationJob createJob(Notebook notebook, Long jobId, String scopeText, int quizCount) {
        GenerationJob job = GenerationJob.create(
                notebook.getUser(), notebook, null, scopeText, "MULTIPLE_CHOICE", "중", quizCount, "qwen3:4b"
        );
    ...
    private Section createSection(Notebook notebook, Long sectionId) {
        ...
        Section section = Section.create(document, new ExtractedSection(
                1, "RAG", "RAG는 검색 증강 생성이다.", SourceUnitType.PAGE, 1, 1, List.of(1)
        ));
